...
export function GroundCombatScene({ missionName, enemyName, hostilesTotal, onHud, onMissionEnd }: GroundCombatSceneProps) {
  void missionName;
  ...
  useThreeScene(
    ...
    ({ scene, camera, quality }) => {
      ...
      scene.background = new THREE.Color(0x2b2416);
      scene.fog = new THREE.Fog(0x4a4530, 25, 65);
      scene.add(new THREE.AmbientLight(0xffe8c0, 0.55));
      const sun = new THREE.DirectionalLight(0xfff0d0, desktop ? 1.1 : 0.9);
      sun.position.set(10, 22, -5);
      scene.add(sun);

      const ground = new THREE.Mesh(new THREE.PlaneGeometry(SPREAD * 3.2, BATTLEFIELD_LEN + 10), new THREE.MeshStandardMaterial({ map: groundTexture(), roughness: 1 }));
      ground.rotation.x = -Math.PI / 2;
      ground.position.set(0, 0, BATTLEFIELD_LEN / 2);
      scene.add(ground);

      // Scattered cover blocks purely for atmosphere.
      const coverMat = new THREE.MeshStandardMaterial({ color: 0x4b463a, roughness: 0.95 });
      for (let i = 0; i < 10; i++) {
